...
export class QrService {
  ...
  payWithQRCode(qrCode: string, qrId: number) : Observable<any>{
    var uri: string = `${this.API_PATH}/payment/qr/pay/${localStorage.getItem("userId")}/${qrId}`
    return this.httpClient.post<any>(uri, qrCode,  { observe: 'response' ,
      // withCredentials: true,
      // headers: {
      //   'Access-Control-Allow-Origin': 'http://localhost:4200' // Your front-end URL})
      // }
    })
  }

  saveCardDto(cc: CardDto) {
    return this.httpClient.post<any>(`${this.API_PATH}/payment/qr/save/${localStorage.getItem("userId")}`, cc)
  }
}
